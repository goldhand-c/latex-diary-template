...
\newcommand\Arg[1]{$\langle\textit{#1}\,\rangle$}

\daynote[\texttt{\string\monthnote}]{2/3/2026}
\texttt{\string\monthnote\{\Arg{month}\}} is used to type the new month header.
It is typed with note paper background in \textbf{bold} font.
It should be used at the start of a page.

\daynote[\texttt{\string\daynote}]{4/3/2026}
\texttt{\string\daynote[\Arg{text}]\{\Arg{dd/mm/yyyy}\}} is used before each day entry to type the date in old-style format and the day of the week.
You can insert an optional argument to note after the day of week.

\daynote[\texttt{\string\xdaynote}]{8/3/2026}
\texttt{\string\xdaynote\{\Arg{text}\}} is used after each day to note extra message.
